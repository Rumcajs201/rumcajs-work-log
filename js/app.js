...
function openOperationForm(type) {
  ...
  $("#operationFormBox").classList.remove("hidden");
  $("#newOperationButtons").classList.add("hidden");
  $("#emptyPalletsLabel").classList.toggle("hidden", type === "load");
  $("#operationPlaceLabel").firstChild.textContent = type === "unload" ? "Sklep / miejsce rozładunku" : "Firma / miejsce załadunku";
  $("#operationPlace").value = ""; $("#operationPallets").value = ""; $("#operationEmptyPallets").value = ""; $("#operationNotes").value = "";
}
function closeOperationForm() { pendingOperationType = null; $("#operationFormBox").classList.add("hidden"); $("#newOperationButtons").classList.remove("hidden"); }

async function confirmOperationStart() {
  const place = $("#operationPlace").value.trim(); if (!place) return toast("Wpisz nazwę miejsca.");
  const existing = await getActiveOperation(dateId()); if (existing) return toast("Jedna operacja jest już rozpoczęta.");
  const now = new Date(); const position = await capturePosition(pendingOperationType === "unload" ? "rozładunek" : "załadunek");
  await startOperation({ workdayId: dateId(), type: pendingOperationType, place, detectedStartTime: clock(now), startTime: roundToNearestFive(now), pallets: $("#operationPallets").value, emptyPallets: $("#operationEmptyPallets").value, notes: $("#operationNotes").value.trim(), position });
  closeOperationForm(); toast(`Operacja rozpoczęta: ${roundToNearestFive(now)}`); await renderOperations();
}

async function finishActiveOperation() { const active = await getActiveOperation(dateId()); if (!active) return; const now = new Date(); await finishOperation(active.id, { detectedEndTime: clock(now), endTime: roundToNearestFive(now) }); toast(`Operacja zakończona: ${roundToNearestFive(now)}`); await renderOperations(); }

async function renderOperations() {
  const items = await getOperationsForDay(dateId()); const active = items.find(item => !item.endTime);
  $("#activeOperationBox").classList.toggle("hidden", !active); $("#newOperationButtons").classList.toggle("hidden", Boolean(active) || Boolean(pendingOperationType));
  ...
  $("#todayOperations").innerHTML = items.length ? items.map(item => `<div class="timeline-item"><strong>${item.type === "unload" ? "Rozładunek" : "Załadunek"}: ${item.place}</strong><span>${item.startTime} – ${item.endTime || "trwa"} • palety: ${item.pallets}${item.type === "unload" ? ` • puste: ${item.emptyPallets}` : ""}</span>${item.notes ? `<span>${item.notes}</span>` : ""}</div>`).join("") : '<p class="muted">Brak operacji w tym dniu.</p>';
}
